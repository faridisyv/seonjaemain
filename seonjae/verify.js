...
async function main() {
  ...
  try {
    ...
    // nudge
    const nudge = await req("POST", `/api/room/${roomId}/nudge`, { role: "A" });
    check("POST nudge returns 200", nudge.status === 200);
    check("nudge is recorded on the room", nudge.json?.room?.nudge?.from === "A");

    const badNudge = await req("POST", `/api/room/${roomId}/nudge`, { role: "Z" });
    check("invalid nudge role rejected with 400", badNudge.status === 400);

    // email registration
    const email = await req("POST", `/api/room/${roomId}/email`, { role: "B", email: "test@example.com" });
    check("POST email returns 200", email.status === 200);
    check("email stored on the room", email.json?.room?.emails?.B === "test@example.com");

    // cron endpoint auth (no CRON_SECRET set in this test run, so it should 403)
    const cronNoKey = await req("GET", "/api/cron/remind?key=wrong");
    check("cron endpoint without valid key returns 403", cronNoKey.status === 403);
  } catch (e) {
